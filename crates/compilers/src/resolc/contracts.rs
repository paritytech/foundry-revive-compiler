...
use foundry_compilers_artifacts::{resolc::contract::ResolcContract, CompactContractRef};
use semver::Version;
...
impl VersionedContracts {
    ...
    pub fn contracts(&self) -> impl Iterator<Item = (&String, &ResolcContract)> {
        self.0
            .values()
    ...
    pub fn into_contracts(self) -> impl Iterator<Item = (String, ResolcContract)> {
    ...
    /// Returns an iterator over (`file`, `name`, `Contract`)
    pub fn into_contracts_with_files(
        self,
    ) -> impl Iterator<Item = (PathBuf, String, ResolcContract)> {
